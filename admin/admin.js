(() => {
  ...
  let applications = [], quotes = [], adminUsers = [], currentAdmin = null, bootstrapSession = false, refreshTimer, activeView = "home", showingArchive = false, sendTarget = null;
  ...
  function switchView(view) {
    ...
    for (const name of ["home", "drivers", "quotes", "users"]) ui[`${name}View`].hidden = name !== view;
  ...
  function renderAll() { renderHome(); renderDrivers(); renderQuotes(); renderAdminUsers(); }
  ...
  async function refreshAll() {
    try {
      const [appsResponse, quotesResponse, usersResponse] = await Promise.all([fetch("/api/admin/applications", { cache: "no-store" }), fetch("/api/admin/quotes", { cache: "no-store" }), fetch("/api/admin/users", { cache: "no-store" })]);
      if (appsResponse.status === 401 || quotesResponse.status === 401 || usersResponse.status === 401) { showLogin(); return; }
      if (!appsResponse.ok || !quotesResponse.ok || !usersResponse.ok) throw new Error();
      const appData = await appsResponse.json(), quoteData = await quotesResponse.json(), userData = await usersResponse.json(); applications = appData.applications || []; quotes = quoteData.quotes || []; adminUsers = userData.users || []; currentAdmin = userData.currentUser || currentAdmin; bootstrapSession = Boolean(currentAdmin?.bootstrap); ui.currentAdminName.textContent = currentAdmin?.fullName || "Administrator"; ui.currentAdminEmail.textContent = currentAdmin?.email || (bootstrapSession ? "First-time setup" : ""); renderAll(); ui.updated.textContent = `Updated ${new Date().toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}`;
